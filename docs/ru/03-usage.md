...
секциями: одиночные подчёркивания внутри имени поля при этом сохраняются.
...
Старые имена с одиночными подчёркиваниями остаются совместимыми для известных
фиксированных ключей. Удалённые API endpoint должны использовать HTTPS,
timeout должен быть положительным, а лимиты включённого кэша проверяются до
