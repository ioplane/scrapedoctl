...
Проект использует сфокусированную CI/CD-конфигурацию:

- **Сборка и тесты** — Go 1.27.0 на Trixie, детектор гонок и атомарное покрытие.
- **golangci-lint v2.13.2** — блокирующий линтинг со строгой конфигурацией v2 из репозитория.
- **UPX-сжатие** — бинарники CI и релизов сжимаются для уменьшения размера загрузки.
- **Локальный аудит безопасности** — `go run ./cmd/devtool audit` запускает Semgrep и Snyk без публикации результатов сканеров в GitHub; точные версии, пороги, доказательства входного корпуса и обработка exit code заданы в [policy локального release gate](../en/05-local-release-gate.md).
