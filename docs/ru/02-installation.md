...
- **Go 1.27+** (для сборки из исходников)
...
Экспортируйте `SCRAPEDO_TOKEN` в окружении, из которого запускается выбранный
агент. В agent config сохраняется ссылка `${SCRAPEDO_TOKEN}`, а не значение
токена. До изменения любого выбранного файла установщик разбирает все agent
config; ошибка парсинга отменяет всю операцию. Успешная замена выполняется
атомарно, а для существующего файла создаётся timestamped backup с правами
`0600` и проверенной контрольной суммой.
